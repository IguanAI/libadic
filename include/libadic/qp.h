...
public:
    ...
    Qp(long p, long N) : prime(p), precision(N), valuation_val(N), unit(p, N, 0) {
        validate();
    }
    
    Qp(long p, long N, const BigInt& val) : prime(p), precision(N) {
        validate();
        if (val.is_zero()) {
            valuation_val = precision;
            unit = Zp(p, N, 0);
        } else {
            valuation_val = p_adic_valuation(val, BigInt(p));
            if (valuation_val >= precision) {
                valuation_val = precision;
                unit = Zp(p, N, 0);
            } else {
                BigInt unit_val = val / BigInt(p).pow(valuation_val);
                unit = Zp(p, N - valuation_val, unit_val);
            }
        }
    }
    
    Qp(long p, long N, long val) : Qp(p, N, BigInt(val)) {}
    ...
    std::string to_string() const {
        ...
        if (valuation_val != 0) {
            result += " * " + std::to_string(prime) + "^" + std::to_string(valuation_val);
        }
