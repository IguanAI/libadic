...
public:
    static Zp gamma(const Zp& x) {
        ...
        if (x_mod_p <= p / 2) {
            for (long k = 1; k < x_mod_p; ++k) {
                result = (result * BigInt(k)) % p_power;
            }
            ...
            for (long k = 1; k < p; ++k) {
                if (k != x_mod_p) {
                    wilson = (wilson * BigInt(k)) % p_power;
                }
            }
        ...
        for (long k = 1; k < N && k < 10; ++k) {
            ...
            BigInt correction = compute_mahler_correction(x_lifted, p, k);
