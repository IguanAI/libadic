...
void bind_qp(py::module_ &m) {
    py::class_<Qp>(m, "Qp", R"pbdoc(
        ...
        .def_static("from_rational", &Qp::from_rational,
            ...
            Returns:
                Qp representing numerator/denominator
                
            Example:
                >>> q = Qp.from_rational(22, 7, 5, 20)  # 22/7 in Q_5
        )pbdoc")
        
        .def_static("from_unit_and_valuation", &Qp::from_unit_and_valuation,
                    py::arg("prime"), py::arg("precision"),
        ...
        .def("norm", [](const Qp &self) {
            ...
            return std::pow(self.get_prime(), -self.valuation());
        }, "p-adic norm |x|_p = p^(-v_p(x))")
